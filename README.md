...
This repo. is no longer supported on github, see here for the new location on gitlab: [https://gitlab.com/yereverluvinunclebert/yereverluvinunclebert](https://gitlab.com/yereverluvinunclebert/gdip-image-list)
...
The reason that this class exists is twofold, i. To make the collection accessible to RC users with very few code changes ii. The class can resize images - TwinBasic currently lacks in-built image resizing.
...
To set the imageWidth, (width in pixels not twips. Note: by default a value of 0, the image's real width will be used)
  
    thisImageList.ImageWidth = 150  ' 
  
To set the imageHeight, (height in pixels not twips. Note: by default a value of 0, the image's real width will be used)
  
    thisImageList.ImageHeight = 150  '  
  
To set the opacity of the image priror to loading, (in the TBImageList, not yet functional):
